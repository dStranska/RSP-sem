...
script("$fh.onclick = function () { return confirm('".($Ke?js_escape($Ke):'Opravdu?')."'); };","");}function
...
lang($ti,$gf=null){if(is_array($ti)){$dg=($gf==1?0:($gf&&$gf<5?1:2));$ti=$ti[$dg];}$ti=str_replace("%d","%s",$ti);$gf=format_number($gf);return
...
query($G,$Ci=false){$H=parent::query($G);$this->error="";if(!$H){list(,$this->errno,$this->error)=$this->errorInfo();if(!$this->error)$this->error='NeznÃ¡mÃ¡ chyba.';return
...
h($g->error);}function
check_sqlite_name($C){global$g;$Kc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Kc)\$~",$C)){$g->error=sprintf('ProsÃ­m pouÅ¾ijte jednu z koncovek %s.',str_replace("|",", ",$Kc));return
false;}return
true;}function
...
Min_DB;$Hb=$b->credentials();if($g->connect($Hb[0],$Hb[1],$Hb[2])){if(min_version(9,0,$g)){$g->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$g)){$Hh['Å˜etÄ›zce'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$g)){$Hh['Å˜etÄ›zce'][]="jsonb";$U["jsonb"]=4294967295;}}}return$g;}return$g->error;}function
get_databases(){return
...
set_schema($Zg){global$g,$U,$Hh;$I=$g->query("SET search_path TO ".idf_escape($Zg));foreach(types()as$T){if(!isset($U[$T])){$U[$T]=0;$Hh['UÅ¾ivatelskÃ© typy'][]=$T;}}return$I;}function
...
unconvert_field($o,$I){return$I;}function
support($Pc){return
preg_match('~^(columns|database|drop_col|indexes|descidx|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Pc);}$x="oracle";$U=array();$Hh=array();foreach(array('ÄŒÃ­sla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a Äas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Å˜etÄ›zce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡rnÃ­'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}$Ii=array();$uf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$kd=array("length","lower","round","upper");$qd=array("avg","count","count distinct","max","min","sum");$mc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ec["mssql"]="MS SQL (beta)";if(isset($_GET["mssql"])){$gg=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
logged_user(){global$b;$Hb=$b->credentials();return$Hb[1];}function
connect(){global$b;$g=new
Min_DB;list($N,$V,$F)=$b->credentials();$xf=array();if($V.$F!=""){$xf["username"]=$V;$xf["password"]=$F;}$l=$b->database();if($l!="")$xf["db"]=$l;if(($La=getenv("MONGO_AUTH_SOURCE")))$xf["authSource"]=$La;try{$g->_link=$g->connect("mongodb://$N",$xf);if($F!=""){$xf["password"]="";try{$g->connect("mongodb://$N",$xf);return'DatabÃ¡ze nepodporuje heslo.';}catch(Exception$Ac){}}return$g;}catch(Exception$Ac){return$Ac->getMessage();}}function
...
last_id(){global$g;return$g->last_id;}$x="elastic";$uf=array("=","query");$kd=array();$qd=array();$mc=array(array("json"));$U=array();$Hh=array();foreach(array('ÄŒÃ­sla'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Datum a Äas'=>array("date"=>10),'Å˜etÄ›zce'=>array("string"=>65535,"text"=>65535),'BinÃ¡rnÃ­'=>array("binary"=>255),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}}$ec["clickhouse"]="ClickHouse (alpha)";if(isset($_GET["clickhouse"])){define("DRIVER","clickhouse");class
...
0;}function
support($Pc){return
preg_match("~^(columns|sql|status|table)$~",$Pc);}$x="clickhouse";$U=array();$Hh=array();foreach(array('ÄŒÃ­sla'=>array("Int8"=>3,"Int16"=>5,"Int32"=>10,"Int64"=>19,"UInt8"=>3,"UInt16"=>5,"UInt32"=>10,"UInt64"=>20,"Float32"=>7,"Float64"=>16,'Decimal'=>38,'Decimal32'=>9,'Decimal64'=>18,'Decimal128'=>38),'Datum a Äas'=>array("Date"=>13,"DateTime"=>20),'Å˜etÄ›zce'=>array("String"=>0),'BinÃ¡rnÃ­'=>array("FixedString"=>0),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}$Ii=array();$uf=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$kd=array();$qd=array("avg","count","count distinct","max","min","sum");$mc=array();}$ec=array("server"=>"MySQL")+$ec;if(!defined("DRIVER")){$gg=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
selectLinks($Oh,$O=""){global$x,$m;echo'<p class="links">';$we=array("select"=>'Vypsat data');if(support("table")||support("indexes"))$we["table"]='Zobrazit strukturu';if(support("table")){if(is_view($Oh))$we["view"]='PozmÄ›nit pohled';else$we["create"]='PozmÄ›nit tabulku';}if($O!==null)$we["edit"]='NovÃ¡ poloÅ¾ka';$C=$Oh["Name"];foreach($we
...
selectColumnsPrint($L,$f){global$kd,$qd;print_fieldset("select",'Vypsat',$L);$s=0;$L[""]=array();foreach($L
as$y=>$X){$X=$_GET["columns"][$y];$e=select_input(" name='columns[$s][col]'",$f,$X["col"],($y!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($kd||$qd?"<select name='columns[$s][fun]'>".optionlist(array(-1=>"")+array_filter(array('Funkce'=>$kd,'Agregace'=>$qd)),$X["fun"])."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($y!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($e)":$e)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($zf,$f,$w){print_fieldset("sort",'SeÅ™adit',$zf);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]'",$f,$X,"selectFieldChange"),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),'sestupnÄ›')."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]'",$f,"","selectAddRow"),checkbox("desc[$s]",1,false,'sestupnÄ›')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($di){if($di!==null){echo"<fieldset><legend>".'DÃ©lka textÅ¯'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($di)."'>","</div></fieldset>\n";}}function
...
messageQuery($G,$ei,$Nc=false){global$x,$m;restart_session();$yd=&get_session("queries");if(!$yd[$_GET["db"]])$yd[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\nâ€¦";$yd[$_GET["db"]][]=array($G,time(),$ei);$_h="sql-".count($yd[$_GET["db"]]);$I="<a href='#$_h' class='toggle'>".'SQL pÅ™Ã­kaz'."</a>\n";if(!$Nc&&($dj=$m->warnings())){$t="warnings-".count($yd[$_GET["db"]]);$I="<a href='#$t' class='toggle'>".'VarovÃ¡nÃ­'."</a>, $I<div id='$t' class='hidden'>\n$dj</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$_h' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($G,1000)."</code></pre>".($ei?" <span class='time'>($ei)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($yd[$_GET["db"]])-1)).'">'.'Upravit'.'</a>':'').'</div>';}function
...
';}$this->databasesPrint($Se);if(DB==""||!$Se){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL pÅ™Ã­kaz'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'Import'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}if($_GET["ns"]!==""&&!$Se&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'VytvoÅ™it tabulku'."</a>\n";if(!$S)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($Se){global$b,$g;$k=$this->databases();if($k&&!in_array(DB,$k))array_unshift($k,DB);echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Pb=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<span title='".'databÃ¡ze'."'>".'DB'."</span>: ".($k?"<select name='db'>".optionlist(array(""=>"")+$k,DB)."</select>$Pb":"<input name='db' value='".h(DB)."' autocapitalize='off'>\n"),"<input type='submit' value='".'Vybrat'."'".($k?" class='hidden'":"").">\n";if($Se!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".'SchÃ©ma'.": <select name='ns'>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>$Pb";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}foreach(array("import","sql","schema","dump","privileges")as$X){if(isset($_GET[$X])){echo"<input type='hidden' name='$X' value=''>";break;}}echo"</p></form>\n";}function
tablesPrint($S){echo"<ul id='tables'>".script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($S
as$Q=>$Eh){$C=$this->tableName($Eh);if($C!=""){echo'<li><a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q,"select").">".'vypsat'."</a> ",(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Eh)?"view":"structure"))." title='".'Zobrazit strukturu'."'>$C</a>":"<span>$C</span>")."\n";}}echo"</ul>\n";}}$b=(function_exists('adminer_object')?adminer_object():new
...
check_invalid_login(){global$b;$Wd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Vd=$Wd[$b->bruteForceKey()];$bf=($Vd[1]>29?$Vd[0]-time():0);if($bf>0)auth_error(lang(array('PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minutu.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minuty.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minut.'),ceil($bf/60)));}$Ka=$_POST["auth"];if($Ka){session_regenerate_id();$Xi=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$F=(string)$Ka["password"];$l=$Ka["db"];set_password($Xi,$N,$V,$F);$_SESSION["db"][$Xi][$N][$V][$l]=true;if($Ka["permanent"]){$y=base64_encode($Xi)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($l);$mg=$b->permanentLogin(true);$ag[$y]="$y:".base64_encode($mg?encrypt_string($F,$mg):"");cookie("adminer_permanent",implode(" ",$ag));}if(count($_POST)==1||DRIVER!=$Xi||SERVER!=$N||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($Xi,$N,$V,$l));}elseif($_POST["logout"]){if($vd&&!verify_token()){page_header('OdhlÃ¡sit','NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡Å¡enÃ­ probÄ›hlo v poÅ™Ã¡dku.'.' '.'DÃ­ky za pouÅ¾itÃ­ Admineru, <a href="https://www.adminer.org/cs/donation/">pÅ™ispÄ›jte</a> na vÃ½voj.');}}elseif($ag&&!$_SESSION["pwds"]){session_regenerate_id();$mg=$b->permanentLogin();foreach($ag
...
auth_error($n){global$b,$vd;$mh=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$mh]||$_GET[$mh])&&!$vd)$n='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{restart_session();add_invalid_login();$F=get_password();if($F!==null){if($F===false)$n.='<br>'.sprintf('Platnost hlavnÃ­ho hesla vyprÅ¡ela. <a href="https://www.adminer.org/cs/extension/"%s>Implementujte</a> metodu %s, aby platilo stÃ¡le.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$mh]&&$_GET[$mh]&&ini_bool("session.use_only_cookies"))$n='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';$Nf=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$Nf["lifetime"]);page_header('PÅ™ihlÃ¡sit se',$n,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'Akce bude provedena po ÃºspÄ›Å¡nÃ©m pÅ™ihlÃ¡Å¡enÃ­ se stejnÃ½mi pÅ™ihlaÅ¡ovacÃ­mi Ãºdaji.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$gg)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Ad,$cg)=explode(":",SERVER,2);if(is_numeric($cg)&&$cg<1024)auth_error('PÅ™ipojovÃ¡nÃ­ k privilegovanÃ½m portÅ¯m nenÃ­ povoleno.');check_invalid_login();$g=connect();$m=new
Min_Driver($g);}$xe=null;if(!is_object($g)||($xe=$b->login($_GET["username"],get_password()))!==true){$n=(is_string($g)?h($g):(is_string($xe)?$xe:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.'));auth_error($n.(preg_match('~^ | $~',get_password())?'<br>'.'ProblÃ©m mÅ¯Å¾e bÃ½t, Å¾e je v zadanÃ©m hesle mezera.':''));}if($Ka&&$_POST["token"])$_POST["token"]=$oi;$n='';if($_POST){if(!verify_token()){$Qd="max_input_vars";$Ie=ini_get($Qd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Ie||$X<$Ie)){$Qd=$y;$Ie=$X;}}}$n=(!$_POST["token"]&&$Ie?sprintf('Byl pÅ™ekroÄen maximÃ¡lnÃ­ povolenÃ½ poÄet polÃ­. ZvyÅ¡te prosÃ­m %s.',"'$Qd'"):'NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.'.' '.'Pokud jste tento poÅ¾adavek neposlali z Adminera, tak tuto strÃ¡nku zavÅ™ete.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=sprintf('PÅ™Ã­liÅ¡ velkÃ¡ POST data. ZmenÅ¡ete data nebo zvyÅ¡te hodnotu konfiguraÄnÃ­ direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.'VelkÃ½ SQL soubor mÅ¯Å¾ete nahrÃ¡t pomocÃ­ FTP a importovat ho ze serveru.';}function
...
edit_type($y,$o,$pb,$ed=array(),$Lc=array()){global$Hh,$U,$Ii,$pf;$T=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" aria-labelledby="label-type">';if($T&&!isset($U[$T])&&!isset($ed[$T])&&!in_array($T,$Lc))$Lc[]=$T;if($ed)$Hh['CizÃ­ klÃ­Äe']=$ed;echo
optionlist(array_merge($Lc,$Hh),$T),'</select>
',on_help("getTarget(event).value",1),script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});",""),'<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3"',(!$o["length"]&&preg_match('~var(char|binary)$~',$T)?" class='required'":"");echo' aria-labelledby="label-length">',script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",""),'<td class="options">',"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$T)?"":" class='hidden'").'><option value="">('.'porovnÃ¡vÃ¡nÃ­'.')'.optionlist($pb,$o["collation"]).'</select>',($Ii?"<select name='".h($y)."[unsigned]'".(!$T||preg_match(number_type(),$T)?"":" class='hidden'").'><option>'.optionlist($Ii,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$T)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'PÅ™i zmÄ›nÄ›'.")","CURRENT_TIMESTAMP"),(preg_match('~^CURRENT_TIMESTAMP~i',$o["on_update"])?"CURRENT_TIMESTAMP":$o["on_update"])).'</select>':''),($ed?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$T)?"":" class='hidden'")."><option value=''>(".'PÅ™i smazÃ¡nÃ­'.")".optionlist(explode("|",$pf),$o["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td id='label-comment'>".'KomentÃ¡Å™':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>".script("row_count = ".count($p).";"),'</thead>
<tbody>
...
';edit_type("fields[$s]",$o,$pb,$ed);if($T=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}echo' aria-labelledby="label-ai"></label><td>',checkbox("fields[$s][has_default]",1,$o["has_default"],"","","","label-default"),'<input name="fields[',$s,'][default]" value="',h($o["default"]),'" aria-labelledby="label-default">',(support("comment")?"<td><input name='fields[$s][comment]' value='".h($o["comment"])."' data-maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'> "."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=4.7.4")."' alt='â†‘' title='".'PÅ™esunout nahoru'."'> "."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=4.7.4")."' alt='â†“' title='".'PÅ™esunout dolÅ¯'."'> ":""),($Cf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.7.4")."' alt='x' title='".'Odebrat'."'>":"");}}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'ZmÄ›nit'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'PÅ™idat trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('SchÃ©ma databÃ¡ze',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Rh=array();$Sh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Ce,PREG_SET_ORDER);foreach($Ce
...
as$C=>$o){if(!isset($o["privileges"][$Ji?"update":"insert"])||$b->fieldName($o)==""||$o["generated"])unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Ji?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$Ei=unique_array($_GET["where"],$w);$wg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'PoloÅ¾ka byla smazÃ¡na.',$m->delete($a,$wg,!$Ei));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Ji){if(!$O)redirect($A);queries_redirect($A,'PoloÅ¾ka byla aktualizovÃ¡na.',$m->update($a,$O,$wg,!$Ei));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$H=$m->insert($a,$O);$oe=($H?last_id():0);queries_redirect($A,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($oe?" $oe":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($p
...
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>",'</noscript>
</thead>
';if($jg){echo"<tr><td>PRIMARY<td>";foreach($jg["columns"]as$y=>$e){echo
select_input(" disabled",$p,$e),"<label><input disabled type='checkbox'>".'sestupnÄ›'."</label> ";}echo"<td><td>\n";}$ce=1;foreach($J["indexes"]as$v){if(!$_POST["drop_col"]||$ce!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ce][type]",array(-1=>"")+$Jd,$v["type"],($ce==count($J["indexes"])?"indexesAddRow.call(this);":1),"label-type"),"<td>";ksort($v["columns"]);$s=1;foreach($v["columns"]as$y=>$e){echo"<span>".select_input(" name='indexes[$ce][columns][$s]' title='".'Sloupec'."'",($p?array_combine($p,$p):$p),$e,"partial(".($s==count($v["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape($x=="sql"?"":$_GET["indexes"]."_")."')"),($x=="sql"||$x=="mssql"?"<input type='number' name='indexes[$ce][lengths][$s]' class='size' value='".h($v["lengths"][$y])."' title='".'DÃ©lka'."'>":""),(support("descidx")?checkbox("indexes[$ce][descs][$s]",1,$v["descs"][$y],'sestupnÄ›'):"")," </span>";$s++;}echo"<td><input name='indexes[$ce][name]' value='".h($v["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ce]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.7.4")."' alt='x' title='".'Odebrat'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ce++;}echo'</table>
</div>
<p>
<input type="submit" value="UloÅ¾it">
<input type="hidden" name="token" value="',$oi,'">
...
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$J["collation"]))$Jh=false;$ne=$l;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($ne),'DatabÃ¡ze byla vytvoÅ™ena.',$Jh);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'DatabÃ¡ze byla zmÄ›nÄ›na.');}}page_header(DB!=""?'PozmÄ›nit databÃ¡zi':'VytvoÅ™it databÃ¡zi',$n,array(),h(DB));$pb=collations();$C=DB;if($_POST)$C=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$pb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$oi,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$J=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Ke=($_POST["drop"]?'CizÃ­ klÃ­Ä byl odstranÄ›n.':($C!=""?'CizÃ­ klÃ­Ä byl zmÄ›nÄ›n.':'CizÃ­ klÃ­Ä byl vytvoÅ™en.'));$A=ME."table=".urlencode($a);if(!$_POST["drop"]){$J["source"]=array_filter($J["source"],'strlen');ksort($J["source"]);$Xh=array();foreach($J["source"]as$y=>$X)$Xh[$y]=$J["target"][$y];$J["target"]=$Xh;}if($x=="sqlite")queries_redirect($A,$Ke,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($J)))));else{$c="ALTER TABLE ".table($a);$fc="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$fc,$A,$Ke);else{query_redirect($c.($C!=""?"$fc,":"")."\nADD".format_foreign_key($J),$A,$Ke);$n='ZdrojovÃ© a cÃ­lovÃ© sloupce musÃ­ mÃ­t stejnÃ½ datovÃ½ typ, nad cÃ­lovÃ½mi sloupci musÃ­ bÃ½t definovÃ¡n index a odkazovanÃ¡ data musÃ­ existovat.'."<br>$n";}}}page_header('CizÃ­ klÃ­Ä',$n,array("table"=>$a),h($a));if($_POST){ksort($J["source"]);if($_POST["add"])$J["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$J["target"]=array();}elseif($C!=""){$ed=foreign_keys($a);$J=$ed[$C];$J["source"][]="";}else{$J["table"]=$a;$J["source"]=array("");}echo'
<form action="" method="post">
';$vh=array_keys(fields($a));if($J["db"]!="")$g->select_db($J["db"]);if($J["ns"]!="")set_schema($J["ns"]);$Cg=array_keys(array_filter(table_status('',true),'fk_support'));$Xh=($a===$J["table"]?$vh:array_keys(fields(in_array($J["table"],$Cg)?$J["table"]:reset($Cg))));$qf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'CÃ­lovÃ¡ tabulka'.": ".html_select("table",$Cg,$J["table"],$qf)."\n";if($x=="pgsql")echo'SchÃ©ma'.": ".html_select("ns",$b->schemas(),$J["ns"]!=""?$J["ns"]:$_GET["ns"],$qf);elseif($x!="sqlite"){$Rb=array();foreach($b->databases()as$l){if(!information_schema($l))$Rb[]=$l;}echo'DB'.": ".html_select("db",$Rb,$J["db"]!=""?$J["db"]:$_GET["db"],$qf);}echo'<input type="hidden" name="change-js" value="">
...
';if($C!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$C));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$Ff="VIEW";if($x=="pgsql"&&$a!=""){$Eh=table_status($a);$Ff=strtoupper($Eh["Engine"]);}if($_POST&&!$n){$C=trim($J["name"]);$Ga=" AS\n$J[select]";$A=ME."table=".urlencode($C);$Ke='Pohled byl zmÄ›nÄ›n.';$T=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$T=="VIEW"&&$Ff=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$Ke);else{$Zh=$C."_adminer_".uniqid();drop_create("DROP $Ff ".table($a),"CREATE $T ".table($C).$Ga,"DROP $T ".table($C),"CREATE $T ".table($Zh).$Ga,"DROP $T ".table($Zh),($_POST["drop"]?substr(ME,0,-1):$A),'Pohled byl odstranÄ›n.',$Ke,'Pohled byl vytvoÅ™en.',$a,$C);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($Ff!="VIEW");if(!$n)$n=error();}page_header(($a!=""?'PozmÄ›nit pohled':'VytvoÅ™it pohled'),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>KaÅ¾dÃ½ch<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ud,$J["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Fh,$J["STATUS"]),'<tr><th>KomentÃ¡Å™<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" data-maxlength="64">
<tr><th><td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Po dokonÄenÃ­ zachovat'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($aa!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$aa));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?$_GET["name"]:$_GET["procedure"]);$Tg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$n){$Cf=routine($_GET["procedure"],$Tg);$Zh="$J[name]_adminer_".uniqid();drop_create("DROP $Tg ".routine_id($da,$Cf),create_routine($Tg,$J),"DROP $Tg ".routine_id($J["name"],$J),create_routine($Tg,array("name"=>$Zh)+$J),"DROP $Tg ".routine_id($Zh,$J),substr(ME,0,-1),'Procedura byla odstranÄ›na.','Procedura byla zmÄ›nÄ›na.','Procedura byla vytvoÅ™ena.',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmÄ›nit funkci':'ZmÄ›nit proceduru').": ".h($da):(isset($_GET["function"])?'VytvoÅ™it funkci':'VytvoÅ™it proceduru')),$n);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Tg);$J["name"]=$da;}$pb=get_vals("SHOW CHARACTER SET");sort($pb);$Ug=routine_languages();echo'
<form action="" method="post" id="form">
<p>NÃ¡zev: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',($Ug?'Jazyk'.": ".html_select("language",$Ug,$J["language"])."\n":""),'<input type="submit" value="UloÅ¾it">
<div class="scrollable">
<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$pb,$Tg);if(isset($_GET["function"])){echo"<tr><td>".'NÃ¡vratovÃ½ typ';edit_type("returns",$J["returns"],$pb,array(),($x=="pgsql"?array("void","trigger"):array()));}echo'</table>
</div>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$da));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$zi=trigger_options();$J=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$zi["Timing"])&&in_array($_POST["Event"],$zi["Event"])&&in_array($_POST["Type"],$zi["Type"])){$of=" ON ".table($a);$fc="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$of:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($fc,$A,'Trigger byl odstranÄ›n.');else{if($C!="")queries($fc);queries_redirect($A,($C!=""?'Trigger byl zmÄ›nÄ›n.':'Trigger byl vytvoÅ™en.'),queries(create_trigger($of,$_POST)));if($C!="")queries(create_trigger($of,$J+array("Type"=>reset($zi["Type"]))));}}$J=$_POST;}page_header(($C!=""?'ZmÄ›nit trigger'.": ".h($C):'VytvoÅ™it trigger'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
<tr><th>ÄŒas<td>',html_select("Timing",$zi["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>UdÃ¡lost<td>',html_select("Event",$zi["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$zi["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$zi["Type"],$J["Type"]),'</table>
...
as$hf=>$Qg){if(preg_match('~^(.+)(\(.*\))?$~U',$hf,$B))grant("REVOKE",array_keys($Qg),$B[2]," ON $B[1] FROM $Ze");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'UÅ¾ivatel byl zmÄ›nÄ›n.':'UÅ¾ivatel byl vytvoÅ™en.'),!$n);if($Gb)$g->query("DROP USER $Ze");}}page_header((isset($_GET["host"])?'UÅ¾ivatel'.": ".h("$ha@$_GET[host]"):'VytvoÅ™it uÅ¾ivatele'),$n,array("privileges"=>array('','OprÃ¡vnÄ›nÃ­')));if($_POST){$J=$_POST;$md=$Xe;}else{$J=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$mf;if($mf!="")$J["hashed"]=true;$md[(DB==""||$md?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0" class="layout">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'OprÃ¡vnÄ›nÃ­'.doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($md
as$hf=>$ld){echo'<th>'.($hf!="*.*"?"<input name='objects[$s]' value='".h($hf)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'DatabÃ¡ze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Bb=>$Xb){foreach((array)$og[$Bb]as$ng=>$ub){echo"<tr".odd()."><td".($Xb?">$Xb<td":" colspan='2'").' lang="en" title="'.h($ub).'">'.h($ng);$s=0;foreach($md
...
<input type="submit" value="UloÅ¾it">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$je=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$je++;}queries_redirect(ME."processlist=",lang(array('Byl ukonÄen %d proces.','Byly ukonÄeny %d procesy.','Bylo ukonÄeno %d procesÅ¯.'),$je),$je||!$_POST["kill"]);}page_header('Seznam procesÅ¯',$n);echo'
<form action="" method="post">
<div class="scrollable">
...
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$G="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($jg&&is_array($_POST["check"]))||$Yd){$H=($_POST["delete"]?$m->delete($a,$jj):($_POST["clone"]?queries("INSERT $G$jj"):$m->update($a,$O,$jj)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$fj="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$H=($_POST["delete"]?$m->delete($a,$fj,1):($_POST["clone"]?queries("INSERT".limit1($a,$G,$fj)):$m->update($a,$O,$fj,1)));if(!$H)break;$za+=$g->affected_rows;}}}$Ke=lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$za);if($_POST["clone"]&&$H&&$za==1){$oe=last_id();if($oe)$Ke=sprintf('PoloÅ¾ka%s byla vloÅ¾ena.'," $oe");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Ke,$H);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n='Ctrl+kliknÄ›te na polÃ­Äko, kterÃ© chcete zmÄ›nit.';else{$H=true;$za=0;foreach($_POST["val"]as$Fi=>$J){$O=array();foreach($J
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$H=$m->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Fi,$p),!$Yd&&!$jg," ");if(!$H)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$za),$H);}}elseif(!is_string($Tc=get_file("csv_file",true)))$n=upload_error($Tc);elseif(!preg_match('~~u',$Tc))$n='Soubor musÃ­ bÃ½t v kÃ³dovÃ¡nÃ­ UTF-8.';else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$H=true;$rb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~',$Tc,$Ce);$za=count($Ce[0]);$m->begin();$M=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$K=array();foreach($Ce[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$M]*)$M~",$X.$M,$De);if(!$y&&!array_diff($De[1],$rb)){$rb=$De[1];$za--;}else{$O=array();foreach($De[1]as$s=>$nb)$O[idf_escape($rb[$s])]=($nb==""&&$p[$rb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$nb))));$K[]=$O;}}$H=(!$K||$m->insertUpdate($a,$K,$jg));if($H)$H=$m->commit();queries_redirect(remove_from_uri("page"),lang(array('Byl importovÃ¡n %d zÃ¡znam.','Byly importovÃ¡ny %d zÃ¡znamy.','Bylo importovÃ¡no %d zÃ¡znamÅ¯.'),$za),$H);$m->rollback();}}}$Ph=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header('Vypsat'.": $Ph",$n);$O=null;if(isset($Rg["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if($ed[$X["col"]]&&count($ed[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$O);if(!$f&&support("table"))echo"<p class='error'>".'NepodaÅ™ilo se vypsat tabulku'.($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($Z,$f,$w);$b->selectOrderPrint($zf,$f,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($di);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$hd=$g->result(count_rows($a,$Z,$Yd,$nd));$E=floor(max(0,$hd-1)/$z);}$dh=$L;$od=$nd;if(!$dh){$dh[]="*";$Cb=convert_fields($f,$p,$L);if($Cb)$dh[]=substr($Cb,2);}foreach($L
as$y=>$X){$o=$p[idf_unescape($X)];if($o&&($Ga=convert_field($o)))$dh[$y]="$Ga AS $X";}if(!$Yd&&$Hi){foreach($Hi
as$y=>$X){$dh[]=idf_escape($y);if($od)$od[]=idf_escape($y);}}$H=$m->select($a,$dh,$Z,$od,$zf,$z,$E,true);if(!$H)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$H->seek($z*$E);$sc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($E&&$x=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last"&&$z!=""&&$nd&&$Yd&&$x=="sql")$hd=$g->result(" SELECT FOUND_ROWS()");if(!$K)echo"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";else{$Qa=$b->backwardKeys($a,$Ph);echo"<div class='scrollable'>","<table id='table' cellspacing='0' class='nowrap checkable'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),"<thead><tr>".(!$nd&&$L?"":"<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);","")." <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ZmÄ›nit'."</a>");$We=array();$kd=array();reset($L);$yg=1;foreach($K[0]as$y=>$X){if(!isset($Hi[$y])){$X=$_GET["columns"][key($L)];$o=$p[$L?($X?$X["col"]:current($L)):$y];$C=($o?$b->fieldName($o,$yg):($X["fun"]?"*":$y));if($C!=""){$yg++;$We[$y]=$C;$e=idf_escape($y);$Bd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Xb="&desc%5B0%5D=1";echo"<th>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",""),'<a href="'.h($Bd.($zf[0]==$e||$zf[0]==$y||(!$zf&&$Yd&&$nd[0]==$e)?$Xb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($Bd.$Xb)."' title='".'sestupnÄ›'."' class='text'> â†“</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.'Vyhledat'.'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");}echo"</span>";}$kd[$y]=$X["fun"];next($L);}}$ue=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
as$y=>$X)$ue[$y]=max($ue[$y],min(40,strlen(utf8_decode($X))));}}echo($Qa?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($K,$ed)as$Ve=>$J){$Ei=unique_array($K[$Ve],$w);if(!$Ei){$Ei=array();foreach($K[$Ve]as$y=>$X){if(!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~',$y))$Ei[$y]=$X;}}$Fi="";foreach($Ei
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&preg_match('~char|text|enum|set~',$p[$y]["type"])&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x!='sql'||preg_match("~^utf8~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$Fi.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$nd&&$L?"":"<td>".checkbox("check[]",substr($Fi,1),in_array(substr($Fi,1),(array)$_POST["check"])).($Yd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Fi)."' class='edit'>".'upravit'."</a>"));foreach($J
...
as$de=>$W)$_.=where_link($s++,$de,$W);}$X=select_value($X,$_,$o,$di);$t=h("val[$Fi][".bracket_escape($y)."]");$Y=$_POST["val"][$Fi][bracket_escape($y)];$nc=!is_array($J[$y])&&is_utf8($X)&&$K[$Ve][$y]==$J[$y]&&!$kd[$y];$ci=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$nc)||$Y!==null){$sd=h($Y!==null?$Y:$J[$y]);echo"<td>".($ci?"<textarea name='$t' cols='30' rows='".(substr_count($J[$y],"\n")+1)."'>$sd</textarea>":"<input name='$t' value='$sd' size='$ue[$y]'>");}else{$ye=strpos($X,"<i>â€¦</i>");echo"<td id='$t' data-text='".($ye?2:($ci?1:0))."'".($nc?"":" data-warning='".h('Ke zmÄ›nÄ› tÃ©to hodnoty pouÅ¾ijte odkaz upravit.')."'").">$X</td>";}}}if($Qa)echo"<td>";$b->backwardKeysPrint($Qa,$K[$Ve]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($K||$E){$Bc=true;if($_GET["page"]!="last"){if($z==""||(count($K)<$z&&($K||!$E)))$hd=($E?$E*$z:0)+count($K);elseif($x!="sql"||!$Yd){$hd=($Yd?false:found_rows($R,$Z));if($hd<max(1e4,2*($E+1)*$z))$hd=reset(slow_query(count_rows($a,$Z,$Yd,$nd)));else$Bc=false;}}$Lf=($z!=""&&($hd===false||$hd>$z||$E));if($Lf){echo(($hd===false?count($K)+1:$hd-$E*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" class="loadmore">'.'NahrÃ¡t dalÅ¡Ã­ data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'NahrÃ¡vÃ¡ se'."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($K||$E){if($Lf){$Fe=($hd===false?$E+(count($K)>=$z?2:1):floor(($hd-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'StrÃ¡nka'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'StrÃ¡nka'."', '".($E+1)."')); return false; };"),pagination(0,$E).($E>5?" â€¦":"");for($s=max(1,$E-4);$s<min($Fe,$E+5);$s++)echo
pagination($s,$E);if($Fe>0){echo($E+5<$Fe?" â€¦":""),($Bc&&$hd!==false?pagination($Fe,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Fe'>".'poslednÃ­'."</a>");}}else{echo"<legend>".'StrÃ¡nka'."</legend>",pagination(0,$E).($E>1?" â€¦":""),($E?pagination($E,$E):""),($Fe>$E?pagination($E+1,$E).($Fe>$E+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'CelÃ½ vÃ½sledek'."</legend>";$cc=($Bc?"":"~ ").$hd;echo
checkbox("all",1,0,($hd!==false?($Bc?"":"~ ").lang(array('%d Å™Ã¡dek','%d Å™Ã¡dky','%d Å™Ã¡dkÅ¯'),$hd):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$cc' : checked); selectCount('selected2', this.checked || !checked ? '$cc' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>ZmÄ›nit</legend><div>
